...
| **S4** | Two PRs. **S4a — done**: R7 one size formatter (`format_size` re-exported by executor as `format_model_size`; the CLI's MiB copy deleted); `STRATA_LOCAL_API_KEY` deleted (`api_key_env_var` is `None` for local, and `requires_api_key` derives from it); R8 rule 14 and closing item 2 rewritten to the grammar plus a pointer at the matrix; clap-parse guard extended to `docs/design/inference*` sketches, every registry `suggested_fix` / `message_template`, and the D8 prompt as shown (it caught `vector add` in the DX design); `strata models …` fixed in `strata-v1-cli-sdk-experience.md` by hand (Q13). **S4b0**: `capability` on a GGUF path claims what the verbs do — a present file every local ability the build has, an absent one nothing (Q15, #3303); one `TaskFacts` derivation on `ResolvedModel`; the matrix gains the law that a local spec's `can_<task>` is true exactly when the verb would go on to load it. Sequenced before S4b because removing the rerankers leaves a path as the only `rank` story. **S4b — done**: dead catalog entries removed (Q3: the two rerankers, #3045; Q14: `qwen3:1.7b:q4_k_m`, #3300) — each now `inference.unknown_model`; `rank` has no catalogued model and a GGUF path is its story (which S4b0 made honest); every rank observer re-pointed at `fake-rank` or a path; nightly `catalog-reachability` job runs `every_catalogued_file_is_published_on_the_hub` — one `HEAD` per catalogued file, redirect unfollowed, size held to the downloader's own `SIZE_TOLERANCE` | none | S4a: #3235, #3257. S4b0: #3303. S4b: #3045, #3300 |
...
Verified against `main` at `98f8f324`; rows updated through S4b.
...
| Catalog / registry | sound predicates; case-insensitive lookup; one directory resolver; **S1**: one registry per runtime, `lookup` the only catalog question, loaders take a path; **S4b**: every catalogued file is published (the two rerankers and `qwen3:1.7b:q4_k_m` removed, #3045 / #3300) and the nightly `catalog-reachability` lane keeps it so — no catalogued rank model until a reranker repo is published | — |
| Availability | **S1**: `resolve` → `ResolvedModel` / `Availability` for every verb, `require_ready` the only renderer; **S2**: `Availability` reaches the wire as `details` and `capability` as `availability` / `pull_spec` / `size_bytes` / `key_env_var` / `config_key` — one `AvailabilityDetails` type, both directions; **S4b0**: `capability`'s `can_*` derive from `ResolvedModel::task_facts` — a present GGUF path claims every local ability the build has, an absent one nothing (Q15, #3303) — and the matrix pins that a local spec claims a task exactly when the verb would load it | — |
...
| Q3 | Dead reranker catalog entries (#3045) | remove them in S4 unless the repos are published first; a catalogued model that cannot be pulled is a false fact — **done** S4b: removed, `rank` is by GGUF path until a reranker repo is published, and the nightly `HEAD` lane admits any future entry |
...
| Q14 | `qwen3:1.7b:q4_k_m` is catalogued but its file is not published (#3300) | remove the variant in S4b, with Q3's rerankers — **decided** 2026-09-10, **done** S4b |
| Q15 | What `capability` claims for a caller-supplied GGUF path (#3303) | a present file claims every local ability this build has: the resolver returns it ready for every use and every verb loads it, so `can_generate` / `can_embed` / `can_rank` / `can_tokenize` are all `true` in a `local` build and `false` otherwise; the file's shape (`embedding_dim`) stays unknown until it is loaded. An absent file claims nothing (Q5: a name for nothing, like an uncatalogued name). One derivation, `ResolvedModel::task_facts`, feeds `capability`, the resolver's task check and the testkit fake — **decided** 2026-09-10, S4b0 |
