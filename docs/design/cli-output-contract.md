...
| Q16b | How a cell carries a value containing the separator, a newline, or bytes that are not text | **Escape the escape — decided 2026-09-13 (#3358 F7/F8).** A cell is injective *and* safe to print, so a consumer can recover what was stored and a terminal never acts on it: `\\`, `\n`, `\t` and `\r` are escaped (the escape character *first*, or a value spelling `\n` would encode to the same six bytes as one containing a newline), and every **other** control character — the rest of C0, DEL, and the C1 block where 0x9B is a single-byte CSI — takes the `\uXXXX` form the `--json` envelope already uses for the same byte, so a reader meets one encoding and not two (#3371); bytes that are not displayable text carry the Q20 marker in **both** layouts, not only human, because bare base64 cannot be told from a value whose text is those characters; and a literal value beginning with the marker is escaped with a leading `\`, which cannot be read as an encoded backslash because that is always doubled. Displayability is *not* UTF-8 validity — a value with any control character other than the three escaped ones is bytes. A serialized JSON document escapes its own control characters and so is already an unambiguous spelling of itself; it passes through unescaped. Applies wherever values share a line: table cells, `key<TAB>value` lines, **receipts and raw identities** (which previously bypassed it entirely). The whole-value `--raw` point read (Q5/#3116) is a separate encoding and stays byte-exact. *One limit remains. (a) closed in 1.2.3 (#3371): only those four characters were escaped, so a value carrying ESC or another control byte reached the terminal intact and was interpreted there. Injective and safe-to-print are different properties and only the first had been established — `displayable_text` did reject every control character, but runs only for `as: bytes` fields, so an ordinary `string` field such as a JSON document id never met it. (b) closed in 1.2.3 (#3372): `raw_scalar` returned a string bare while serializing structures, so the array `[1]` and the string `"[1]"` produced the same cell, as did `1`/`"1"`, `true`/`"true"`, and an object beside the string spelling it. A JSON-valued cell is now the value's own compact JSON, type included, in **both** layouts — the reason the Q20 marker is in both. So a `--raw json scan` string cell gained its quotes (`abc` became `"abc"`), which is a visible change to that one surface; the separately specified bare leaf of a whole-value `--raw json get` (Q5/#3116) is a different path and did not move. A JSON `null` stays the null cell rather than spelling itself: the wire uses null for "nothing here" — a `batch_get` miss carries `found: false` beside `value: null` — so spelling it as a value would report a document containing null where there is no document, and the record's own status column already carries that distinction. What made `null` and `""` one cell was the empty string being written bare, and quoting it is what separates them.* |
