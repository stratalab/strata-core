...
| Q5 | `Pretty` and the hidden `--output-format` flag (`options.rs:56-59`, "transitional") | **deleted at S4.** *Consequence recorded on landing: a line inside a session can choose `--json` or `--raw`, and there is no longer any way to ask one line for human output — the hidden flag was the only spelling of it. A `--human` flag would restore the symmetry if anyone wants it.* Originally: delete both in S4: `--json \| jq .` is pretty; a hidden second way to choose the format is Rule 8's smell. Release note |
