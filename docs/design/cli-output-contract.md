...
| `BatchResult<T>` | a table, one row per item: `#  STATUS  [EFFECT]  <declared columns>` (+ `ERROR` when any item failed); a stderr summary `-- itemwise: 2 ok, 1 miss` **only when the engine's top-level `status` is not `ok`** (`partial` / `failed` — an item missed or failed, or an atomic batch did not apply); full success is the table alone (Q11). The signal is `status`, never `applied` — every read batch carries `applied: false` because a read applies nothing (review round 3 caught the catalog getting this wrong) | the same rows as TSV | pretty JSON → table |
