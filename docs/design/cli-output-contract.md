...
**What does not change:** `--json` and `--pretty` (if kept, Q5) are
...
| **S4** | R4 raw bytes verbatim for KV reads (the `base64:` labelling in human mode landed with S3a's `as: bytes`, and S3b deleted the date pre-pass); Q5 (`--output-format` / `pretty`) | hidden flag removal if Q5 says so (release note) | #3116 |
