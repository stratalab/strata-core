...
```
Epic 1: Key Encoding & Memtable         ← foundation, no disk I/O               ✅ DONE
...
Epic 9: Shared Segments & Branch Manifests ← O(1) fork, cross-branch compaction
```

Each epic builds on the previous. Each has a clear "done" gate: all tests (unit + contract + integration + regression) pass.

### Epic 9 Design Notes: Shared Segments & Branch Manifests

**Goal:** Eliminate disk amplification from per-branch segment duplication. Branches share
immutable segments via manifests; only branch-local writes (overlay segments) are unique.
...
Fork = flush source memtable + clone manifest + bump ref counts. Zero data copy.

**Why this is safe to defer:**

Epics 1-8 build per-branch segments with a branch-agnostic segment format (no branch_id
in InternalKey encoding within segments). The segment files are already shareable — the
only thing missing is the manifest layer that enables sharing.

**What Epic 9 adds (~1,000 lines, isolated):**

| Component | Lines | Touches |
|-----------|-------|---------|
| `BranchManifest` struct (segment refs + ref counts) | ~200 | New file |
| Manifest persistence (atomic write + CURRENT pointer) | ~200 | New file |
| Fork: flush source memtable → clone manifest → bump refs | ~100 | `branch_store.rs` |
| Compaction prune floor coordination | ~400 | `compaction.rs` only |
| Segment GC (ref_count=0 → delete) | ~100 | `compaction.rs` |

**What doesn't change:**

- Segment format (Epic 2) — untouched, already branch-agnostic
- Memtable (Epic 1) — untouched
- Flush pipeline (Epic 4) — produces same files, registers in manifest instead of directory
- Read path — still memtable → immutable memtables → segments newest-first
- Block cache, WAL, recovery — untouched

**Fork semantics (force-flush approach):**

```
fork(source → dest):
  1. Flush source's active memtable to segment (fast sequential write, ~100ms for 128MB)
  2. Clone source's manifest → dest manifest
  3. Increment ref_count on all shared segments
  4. dest gets a fresh empty memtable
```

Force-flushing before fork makes unflushed memtable data visible in the new segment,
eliminating the P0 correctness issue identified in the design review. Fork cost is
O(flush) not O(1), but O(flush) << O(N data copy) for any non-trivial dataset.
...
This is the hardest code (~400 lines) but is entirely isolated to the compaction module.
The read path, write path, and segment format are unaffected.
...
**Reuse existing `BackgroundScheduler` (issue #1408).** `crates/engine/src/background.rs` already has a priority queue, worker thread pool, backpressure (`max_queue_depth`), panic safety (`catch_unwind` + RAII guard), drain/shutdown — all wired into `Database::shutdown()`/`Drop`. The scheduler's `submit()` method has zero production callers. Epic 4 wires it in for the first time.

- Flush jobs use `TaskPriority::High` (user-initiated flushes) or `TaskPriority::Normal` (rotation-triggered flushes)
- Backpressure from the scheduler maps to write stalling: when `submit()` returns `BackpressureError`, the write path blocks or returns an error
- No new threading infrastructure. The existing 2-thread pool is sufficient for flush; can be tuned via config later.

**`write_buffer_size` is configurable via `StorageConfig`.** Not hardcoded. Default 128 MB, with presets for constrained (16 MB) and server (256 MB) workloads. Needs production tuning.

### 4.1 StorageConfig Extension

**File:** `crates/engine/src/database/config.rs` (modify, ~30 lines)

Add `write_buffer_size` and `max_immutable_memtables` to `StorageConfig`:

```rust
pub struct StorageConfig {
    // ... existing fields ...

    /// Memtable size in bytes before rotation triggers flush.
    /// Default: 128 MB. Tunable for workload — smaller = more frequent flushes
    /// but lower memory, larger = fewer flushes but more WAL to replay on crash.
    #[serde(default = "default_write_buffer_size")]
    pub write_buffer_size: usize,

    /// Maximum number of frozen (immutable) memtables per branch before
    /// writes stall. Backpressure safety valve.
    /// Default: 4. Set to 0 for unlimited (not recommended).
    #[serde(default = "default_max_immutable_memtables")]
    pub max_immutable_memtables: usize,
}

fn default_write_buffer_size() -> usize { 128 * 1024 * 1024 }  // 128 MB
fn default_max_immutable_memtables() -> usize { 4 }
```

### 4.2 Flush Coordinator

**File:** `crates/storage/src/flush.rs` (new, ~300 lines)
**Spec:** [memtable-flush-file.md §4-6](memtable-flush-file.md) (simplified: no flush file)

The flush coordinator is a set of methods on `SegmentedStore`, not a separate struct. Core operations:

- `rotate_memtable(branch_id)` — freeze active, swap in new, push old to `frozen` list
- `flush_frozen(branch_id, dir)` — take oldest frozen memtable, build segment via `SegmentBuilder`, add to `segments` list, drop frozen memtable
- `maybe_rotate_after_write(branch_id)` — called after each write, checks `approx_bytes >= write_buffer_size`
...
fn flush_moves_data_from_memtable_to_segment() {
    // Write 100 entries → manual flush_frozen()
    // Memtable should be empty (new active)
    // All data still readable via get_versioned
    // branch_segment_count() == 1
}

#[test]
fn flush_produces_valid_segment_file() {
    // After flush, .sst file exists with valid header/footer
    // KVSegment::open() succeeds on the produced file
}
...
fn memtable_rotates_when_size_exceeded() {
    // Config with write_buffer_size = 1024 (tiny)
    // Write enough to exceed threshold
    // frozen list has >= 1 memtable OR segment_count >= 1
}

#[test]
fn rotation_creates_fresh_active_memtable() {
    // After rotation, active memtable is empty
    // New writes go to the fresh memtable
    // Old data still readable from frozen memtable
}
...
fn mvcc_correct_with_data_split_across_memtable_and_segment() {
    // Write k=10 at v1 → flush → write k=20 at v2
    // get(k, snapshot=1) → 10 (from segment)
    // get(k, snapshot=2) → 20 (from memtable)
}

#[test]
fn prefix_scan_correct_after_flush() {
    // Write "user:1", "user:2" → flush → write "user:3"
    // scan_prefix("user:") returns all 3 from both segment and memtable
}

#[test]
fn get_history_spans_memtable_and_segment() {
    // Write k at v1, v2 → flush → write k at v3
    // get_history(k) returns [v3, v2, v1] in order
}
...
fn writes_stall_when_max_immutable_memtables_reached() {
    // Config: max_immutable_memtables = 1, no background flush
    // Fill memtable → rotate → fill again → rotate
    // Second rotation should return backpressure error
}
...
fn multiple_sequential_flushes_produce_multiple_segments() {
    ...
    // 3 segments, all data readable, correct MVCC ordering
}

#[test]
fn segment_ordering_newest_first() {
    // After multiple flushes, point lookup checks newest segment first
    // Verify by writing same key across flushes — latest value wins
}
...
fn reads_never_block_during_flush() {
    // Writer fills memtable, manual flush on separate thread
    // Reader thread reads continuously — must never block or see stale data
}

#[test]
fn concurrent_flush_and_write_correctness() {
    // Writer + flusher running concurrently
    // After both finish: all data readable, no lost entries
}
...
fn crash_during_flush_no_data_loss() {
    // Write → start flush → simulate crash (drop mid-flush)
    // .tmp file exists, no .sst yet
    // Reopen → .tmp cleaned up, frozen memtable still intact
}
```

### 4.3 Wire BackgroundScheduler to Flush

**File:** `crates/engine/src/database/mod.rs` (modify, ~50 lines)

This is the integration point that connects the storage-layer flush coordinator
to the engine-layer `BackgroundScheduler` (issue #1408). The scheduler already
exists in `Database`, is exposed via `db.scheduler()`, and is drained/shut down
in `Database::shutdown()`/`Drop`. It just has zero `submit()` calls today.

Wiring:
- After `put_with_version_mode` / `apply_batch`, check `memtable.approx_bytes() >= config.storage.write_buffer_size`
- If exceeded: call `store.rotate_memtable(branch_id)`, then `scheduler.submit(TaskPriority::Normal, move || store.flush_frozen(branch_id, dir))`
- If `submit()` returns `BackpressureError`: log warning, write still succeeds (flush will catch up)
- User-initiated flush via `store.flush_branch()`: uses `TaskPriority::High`
...
#[test]
fn scheduler_submit_is_called_on_memtable_rotation() {
    // Database with small write_buffer_size
    // Write enough to trigger rotation
    // scheduler.stats().tasks_completed >= 1 after drain()
}

#[test]
fn background_flush_triggers_when_memtable_full() {
    // Database with write_buffer_size = 4096
    // Write enough to fill it
    // scheduler.drain()
    // Verify: segment exists, memtable size reduced
}

#[test]
fn background_flush_does_not_block_foreground_writes() {
    // Measure write latency during active flush
    // Writes continue while flush runs on scheduler thread
    // No timeout or deadlock
}

#[test]
fn scheduler_drain_completes_pending_flushes() {
    // Trigger multiple rotations rapidly
    // scheduler.drain() → all flushes complete
    // All data readable from segments
}

#[test]
fn scheduler_shutdown_flushes_remaining_before_exit() {
    // Queue several flush jobs
    // Database::shutdown() calls scheduler.drain() then scheduler.shutdown()
    // All flush jobs completed — no data in frozen memtables
}

#[test]
fn backpressure_when_flush_falls_behind() {
    // Tiny write_buffer_size + slow flush (sleep in flush callback)
    // Write rapidly to trigger many rotations
    // Eventually scheduler queue fills → BackpressureError logged
    // Writes still succeed (rotation queued, flush catches up after drain)
}
```
