...
- **Post-audit fix — KV/JSON session bypass**: An external audit after Phase 5 found that Phase 3 fixed the session-dispatch `as_of` drop bug for Event but the same class of bug still existed for KV and JSON. `dispatch_in_txn` matched `Command::KvGet { key, .. }`, `KvList { .. }`, `JsonGet { key, path, .. }`, and `JsonList { .. }` without branching on `as_of`, routing them to the transaction snapshot reader regardless. Fixed by adding a bypass arm that forwards `as_of: Some(_)` variants of all four commands to `executor.execute(cmd)`, matching the Event/Graph/Vector pattern. Added 4 regression tests that verify each bypass honors `as_of=0` against a committed write plus a staged txn write; confirmed the tests fail without the fix and pass with it. Audit also flagged a smaller contract mismatch: `KvGetv` and `JsonGetv` exposed an `as_of` field that the executor silently discarded — removed the field from both command variants to tighten the contract.
