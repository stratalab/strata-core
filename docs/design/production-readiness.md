...
| E-4 | ~~Vector recovery requires manual `register_vector_recovery()`~~ **RESOLVED** in #2354 (Epics 1-5). Executor's `default_product_spec()` installs `[VectorSubsystem, SearchSubsystem]` through `OpenSpec`, so vector recovery + drop-freeze run automatically on every production-opened database. | **RESOLVED** | `executor/src/api/mod.rs:108` | — | #2354 |
