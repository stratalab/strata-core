...
mod mutation_gate_error_value {
    ...
    fn configured_expression_is_constructible_and_verbatim() {
        assert!(!format!(
            "{:?}",
            crate::InferenceError::InvalidSpec(String::from("mutated"))
        )
        .is_empty());
        let declared = |expression: &str| {
            include_str!("../../../.cargo/mutants-inference.toml")
                .lines()
                .map(str::trim)
        ...
        assert!(
            declared(r#"crate::InferenceError::InvalidSpec(String::from("mutated"))"#),
            "lane C's error_values must carry this mirror verbatim, uncommented"
        );
    }
}
