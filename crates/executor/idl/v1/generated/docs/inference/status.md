...
`config_file` names the file those settings are read from and whether it can be used — `absent`, `readable`, `unreadable` or `malformed`. This is what separates a provider that was never configured from one whose stored key cannot be reached: every settings read folds a failure into "no value", so a key in a malformed file reports exactly like no key at all (`key_present: false`, `key_source: null`), and the remedy a caller would offer — set a key — is the wrong one, because a key is already there. The four words are the ones `strata doctor` prints for the same file. `config_file` is `null` when the runtime reads no file at all — an embedding application supplying its own settings, or a platform with no user config directory — which is a different answer from `absent`, a path with no file at it.
