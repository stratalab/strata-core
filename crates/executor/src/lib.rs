...
mod mutation_gate_error_value {
    ...
    fn configured_expression_is_constructible_and_verbatim() {
        assert!(!format!("{:?}", crate::ExecutorError::new("mutated", "mutated")).is_empty());
        let declared = |expression: &str| {
            include_str!("../../../.cargo/mutants-executor.toml")
                .lines()
                .map(str::trim)
        ...
        assert!(
            declared(r#"crate::ExecutorError::new("mutated", "mutated")"#),
            "lane B's error_values must carry this mirror verbatim, uncommented"
        );
    }
}
