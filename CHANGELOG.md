...
hand-written per command, and a single resolver behind every inference model
reference.

Until now each command decided for itself what a person saw, and 23 renderer
arms had grown apart: some printed a receipt, some a JSON dump, some a bare
`true`. Every command now declares what it shows — which facts, in what order,
how a date or a byte count or a set of bytes is presented — and one renderer
reads that declaration. `--raw` is the shell-composable form of the same
declared facts; `--json` is the unmodified wire record, as before.

### Added

- `--human` names the default output mode, so a REPL or pipe line can ask for
  it explicitly where `--json` and `--raw` were previously the only names.
- `STRATA_LOG` turns on diagnostic logging to stderr (`error`, `warn`, `info`,
  `debug`, `trace`). `--help` gained an `environment:` section naming it and
  `STRATA_DB`.
- A history row that is a deletion shows `(deleted)` where its value would be,
  distinct from a document whose stored value is `null`.
- An answer cut short says so on stderr — `-- truncated`, `-- showing N of M`
  — and a complete answer stays quiet.
- `strata agents errors` and the reference pages carry per-command transcripts
  showing what each example prints, generated from a real replay.

### Changed

- **`json get --as-of` answers the versioned envelope.** A historical read
  returned `json_value` while a live read of the same command returned
  `json_versioned_value`, so one command carried two wire shapes and the
...
- Human output is declared per command rather than hand-written: writes print
  ...
  commands' human or `--raw` output changes.
- `--raw` of a stored value is now the bytes as stored, with no trailing
  newline of the CLI's own, so `strata --raw kv get k > payload.bin` is the
  file that was stored.
- `--raw` cells are reversible: `\`, `\n`, `\t` and `\r` are escaped, and a
  value that is not displayable text carries a `base64:` marker in both human
  and raw output rather than bare base64 a reader could not tell from text.
- A human table pads to at most 160 columns, so one very wide value no longer
  pads every other row with it; the value still prints in full. Widths are
  measured in terminal columns, so CJK and combining characters align.
- Dates print as `2026-09-10 20:19:44.123456 UTC` — UTC, microsecond
  precision — and read back through `--as-of-time` to the same commit.
- Byte sizes use one vocabulary throughout: decimal units down to the
  kilobyte, `B` below that.
- The boundary error log, which correlates the reference id in an error with
  its underlying cause, is no longer written unless `STRATA_LOG` asks for it.
  Under `--json`, stderr is the error envelope alone, so `2>&1 | jq .error`
  parses.
- A per-line `--branch` in a session applies to that line only; it previously
  became the session's default, so later commands read and wrote the wrong
  branch.
- Inference model references resolve through one resolver, and a model name
  with no provider prefix is a local model rather than an unknown provider.
  Provider keys and base URLs are read from injected settings, so an inference
  one-shot needs no database.

### Removed

- `--output-format` and the `pretty` format. `--json` is the envelope;
  `--pretty` was a second spelling of it.

### Fixed

- Commands that run before a database opens — `config show`/`path`/`set`/
  `unset`, `start`, `stop`, `hub`, `clone` — refused with an error instead of
  panicking the session with exit 101.
- A key containing a newline or tab no longer breaks a receipt across lines or
  a `--raw` record across columns.
- `branch diff` no longer prints an internal vector identity's control bytes to
  the terminal; it carries a `base64:` marker.
- A `--raw` read of a stored empty value is visible in a session transcript
  rather than contributing no line at all.
- The engine's error registry supplies each code's retry policy and suggested
  fix, and every engine error crossing the executor boundary carries its row.
- An IPC connection is introduced before its hello is answered.
