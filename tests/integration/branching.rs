...
fn test_merge_branches_strict() {
    ...
    let result = test_db.db.branches().merge_with_options(
        "source",
        "target",
        MergeOptions::with_strategy(MergeStrategy::Strict),
    );
...
fn vector_merge_dimension_mismatch_rejected() {
    ...
    // of strategy.
    let lww_result = test_db.db.branches().merge_with_options(
        "source",
        "target",
        MergeOptions::with_strategy(MergeStrategy::LastWriterWins),
    );
    ...
    let strict_result = test_db.db.branches().merge_with_options(
        "source",
        "target",
        MergeOptions::with_strategy(MergeStrategy::Strict),
    );
...
fn json_merge_same_path_different_values_lww() {
    ...
    // Strict refuses.
    let strict = test_db.db.branches().merge_with_options(
        "source",
        "target",
        MergeOptions::with_strategy(MergeStrategy::Strict),
    );
...
fn dag_records_fork_with_version() {
    ...
    test_db
        .db
        .branches()
        .fork("dag_fork_src", "dag_fork_dst")
        .unwrap();
...
fn dag_records_merge_with_version_and_keys() {
    ...
    test_db
        .db
        .branches()
        .fork("dag_merge_target", "dag_merge_source")
        .unwrap();
...
fn dag_records_cherry_pick() {
    ...
    test_db
        .db
        .branches()
        .fork("dag_cp_target", "dag_cp_source")
        .unwrap();
...
fn dag_repeated_merge_advances_merge_base() {
    ...
    test_db
        .db
        .branches()
        .fork("dag_rep_target", "dag_rep_source")
        .unwrap();
...
fn dag_survives_database_reopen() {
    let mut test_db = TestDb::new();
    {
        ...
        test_db
            .db
            .branches()
            .fork("dag_persist", "dag_persist_fork")
            .unwrap();
    }
...
fn dag_records_merge_metadata_end_to_end() {
    ...
    test_db
        .db
        .branches()
        .fork("dag_meta_target", "dag_meta_source")
        .unwrap();
