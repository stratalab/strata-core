...
use strata_engine::{JsonValue, SearchRequest};
...
fn test_substrate_orchestrates_primitives() {
    ...
    p.kv.put(&branch_id, "topic1", strata_core::Value::String("machine learning algorithm".into()))
        .expect("kv");
    let doc_id = "ml-doc";
    p.json.create(&branch_id, "default", doc_id, JsonValue::from(serde_json::json!({
        "title": "Introduction to Machine Learning",
        "content": "ML algorithms are powerful"
    }))).expect("json");

    // Substrate should combine results from all primitives via RRF fusion.
    let req = SearchRequest::new(branch_id, "machine learning");
...
fn test_rrf_fusion_multiple_sources() {
    ...
    // Add data to KV
    p.kv.put(&branch_id, "doc1", strata_core::Value::String("the quick brown fox".into()))
        .expect("kv");
    p.kv.put(&branch_id, "doc2", strata_core::Value::String("lazy brown dog".into()))
        .expect("kv");

    let req = SearchRequest::new(branch_id, "brown");
    let response = substrate_search(&test_db.db, &req);
